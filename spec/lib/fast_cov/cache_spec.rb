...
RSpec.describe FastCov::Cache do
  let(:cov) do
    FastCov::Coverage.new(root: fixtures_path("app"))
  end
  ...
  describe ".data=" do
    ...
    it "replaces the cache contents" do
      FastCov::Cache.data = { const_locations: { "Foo" => "/fake.rb" } }
      expect(FastCov::Cache.data[:const_locations]).to have_key("Foo")
    end
  end
  ...
  describe ".clear" do
    it "empties the cache" do
      ...
      expect(FastCov::Cache.data[:const_locations]).not_to be_empty
